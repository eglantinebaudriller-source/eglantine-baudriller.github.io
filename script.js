...
/********************************************************************
 *  HISTORIQUE – ANIMATION TEMPORELLE
 ********************************************************************/
function initHistorique() {

  const DEFAULT_CENTER = [105.4231, -6.1020];
  const DEFAULT_ZOOM = 5;
  const MAX_TIME = 220000;

  const map = new maplibregl.Map({
    container: 'historique-map',
    style: 'https://tiles.openfreemap.org/styles/liberty',
    center: DEFAULT_CENTER,
    zoom: DEFAULT_ZOOM
  });

  mapRegistry["historique"].map = map;
  ...
  const timeSlider = document.getElementById('hist-time-slider');
  const speedSlider = document.getElementById('hist-speed-slider');
  const timeDisplay = document.getElementById('hist-time-display');
  const dateDisplay = document.getElementById('hist-date-display');
  const speedLabel = document.getElementById('hist-speed-label');
  ...
  function updateMap() {
    map.setFilter('events', ['<=', ['get', 'time'], currentTime]);

    const visible = featuresCache
      .filter(f => f.properties.time <= currentTime)
      .sort((a, b) => b.properties.time - a.properties.time)[0];

    if (visible && visible.properties.time !== lastPopupTime) {
      lastPopupTime = visible.properties.time;
      dateDisplay.textContent = `${visible.properties.date} – ${visible.properties.heure}`;
      popup
        .setLngLat(visible.geometry.coordinates)
        .setHTML(`<strong>${visible.properties.date}</strong><br>${visible.properties.desc ?? ''}`)
        .addTo(map);
    }
  ...
  map.on('load', async () => {
    const data = await fetch('geojson/evenements_pendant_4326.geojson').then(r => r.json());
    ...
    map.addLayer({
      ...
      filter: ['<=', ['get', 'time'], -1],
      paint: {
        'circle-radius': ['interpolate', ['linear'], ['zoom'], 6, 4, 10, 7, 14, 12],
        'circle-color': [
          'match',
          ['to-number', ['get', 'type']],
          1, '#FF0000',
  ...
  function animate() {
    currentTime += animationSpeed / 60;
    if (currentTime > MAX_TIME) return;
    updateMap();
    animationId = requestAnimationFrame(animate);
  }

  document.getElementById('hist-play').onclick = () => {
    if (!animationId) animationId = requestAnimationFrame(animate);
  };

  document.getElementById('hist-pause').onclick = () => {
    cancelAnimationFrame(animationId);
    animationId = null;
  };

  document.getElementById('hist-reset').onclick = () => {
    cancelAnimationFrame(animationId);
    animationId = null;
    currentTime = 0;
    lastPopupTime = null;
  ...
  document.getElementById('hist-reset-zoom').onclick = () => {
